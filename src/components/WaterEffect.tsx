  ...
  useEffect(() => {
    ...
    const checkFontsLoaded = () => {
      return new Promise<void>((resolve) => {
        console.log('Checking if Voyage fonts are loaded...');
        
        // Try to load fonts explicitly first
        if (document.fonts && document.fonts.ready) {
          document.fonts.ready.then(() => {
            console.log('Document fonts ready, checking Voyage...');
            checkVoyageFonts();
          });
        } else {
          // Fallback: wait a bit and then check
          setTimeout(() => {
            console.log('Using fallback font check...');
            checkVoyageFonts();
          }, 500);
        }
        
        const checkVoyageFonts = () => {
          if (document.fonts && document.fonts.check) {
            const regularLoaded = document.fonts.check('12px Voyage');
            const boldLoaded = document.fonts.check('bold 12px Voyage');
            
            console.log('Voyage regular loaded:', regularLoaded);
            console.log('Voyage bold loaded:', boldLoaded);
            
            if (regularLoaded && boldLoaded) {
              console.log('Both Voyage fonts are loaded!');
              resolve();
            } else {
              console.log('Fonts not ready, waiting...');
              setTimeout(checkVoyageFonts, 200);
            }
          } else {
            console.log('document.fonts.check not available, using timeout fallback');
    ...
    // Preload Voyage fonts to ensure they're available
    const preloadFonts = () => {
      const fontFace = new FontFace('Voyage', 'url(/fonts/voyage-regular.otf)');
      const fontFaceBold = new FontFace('Voyage', 'url(/fonts/voyage-bold.otf)', { weight: 'bold' });
      ...
      Promise.all([
        ...
        console.log('Voyage fonts preloaded and added to document.fonts');
      }).catch(error => {
        console.log('Font preloading failed:', error);
      });
    };
    
    // Try to preload fonts first
    preloadFonts();
    ...
    checkFontsLoaded().then(() => {
      ...
      gradient.addColorStop(1, '#c9e2fd'); // light blue at bottom
      ...
      ctx.font = '300 32px Gravita, sans-serif'; // font-gravita, font-light, text-base
      ...
      ctx.font = '45px Voyage, serif'; // font-voyage, much bigger than before
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ...
      ctx.font = '600 32px Gravita, sans-serif'; // Bigger professional subtitle font
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ...
      ctx.fillStyle = '#B76E79'; // blush-rosegold color
      ctx.font = '600 32px Gravita, sans-serif'; // Same font and size as technologies subtitle
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.letterSpacing = '1px'; // Subtle letter spacing
      ...
      skills.forEach((skill, index) => {
        ...
        ctx.font = '600 28px Gravita, sans-serif'; // Bigger, bold font
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
